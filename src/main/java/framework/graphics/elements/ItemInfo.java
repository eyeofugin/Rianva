...
public class ItemInfo extends GUIElement {
  ...
  public int[] render() {
    ...
    int[] itemNamePixels =
        getTextLine(
            this.equipment.getName(), this.width, 8, TextAlignment.LEFT, Color.VOID, Color.WHITE);
    ...
    if (!statBonusString.isEmpty()) {
      int[] statPixels =
          getTextLine(statBonusString, this.width, 8, TextAlignment.LEFT, Color.VOID, Color.WHITE);
      fillWithGraphicsSize(0, yf, this.width, 8, statPixels, false);
      yf += 10;
    }
    int[] descriptionPixels =
        getTextBlock(this.equipment.getDescription(), this.width, Color.VOID, Color.WHITE);
