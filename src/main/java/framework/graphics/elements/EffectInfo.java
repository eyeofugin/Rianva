...
public class EffectInfo extends GUIElement {
  ...
  public int[] render() {
    int yf = 0;
    int[] effectNamePixels =
        getTextLine(this.effect.name, this.width, 8, TextAlignment.LEFT, Color.VOID, Color.WHITE);
    fillWithGraphicsSize(0, yf, this.width, 8, effectNamePixels, false);
    String detail = this.effect.getDetailInfo();
    if (!detail.isBlank()) {
      int[] detailPixels =
          getTextLine(detail, this.width, 8, TextAlignment.RIGHT, Color.VOID, Color.WHITE);
    ...
    if (this.effect.origin != null) {
      int[] originPixels =
          getTextLine(
              "From:" + this.effect.origin.getName(),
              this.width,
              8,
              TextAlignment.LEFT,
              Color.VOID,
    ...
    int[] descriptionPixels =
        getTextBlock(this.effect.description, this.width, Color.VOID, Color.WHITE);
