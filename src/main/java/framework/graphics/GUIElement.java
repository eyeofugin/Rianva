...
public class GUIElement {
  ...
  public static void setBackGround(int[] pixels, Color color) {
    ...
    for (GUIElement child : children) {
      if (child != null) {
        ...
        fillWithGraphicsSize(
            child.getX(), child.getY(), child.getWidth(), child.getHeight(), childP, false);
      }
    }
  }
  ...
  protected int[] getTextLine(String text, int textWidth, int textHeight, Color fontColor) {
    return editor.getTextLineNew(
        text, textWidth, textHeight, 1, TextAlignment.CENTER, Color.VOID, fontColor);
  }
  ...
  protected int[] getSmallNumTextLine(
      String text,
      int textWidth,
      int textHeight,
      TextAlignment alignment,
      Color background,
      Color fontColor) {
    return editor.getSmallNumTextLine(
        text, textWidth, textHeight, alignment, background, fontColor);
  }

  protected void writeLine(String text, int xFrom, int xUntil, int yFrom, int yUntil) {
    writeLine(
        text, xFrom, xUntil, yFrom, yUntil, 0, TextAlignment.CENTER, Color.BLACK, Color.WHITE);
  }
  ...
  public static void staticFillSize(
      int xf, int yf, int w, int h, int targetW, int[] target, int[] fill) {
  ...
  protected void fillWithGraphicsSize(
      int xf, int yf, int w, int h, int[] graphics, Color borderColor) {
    fillWithGraphicsSize(xf, yf, w, h, graphics, borderColor != null, borderColor, Color.VOID);
  }

  protected void fillWithGraphicsSize(
      int xf, int yf, int w, int h, int[] graphics, boolean bordered, Color borderColor) {
    fillWithGraphicsSize(xf, yf, w, h, graphics, bordered, borderColor, Color.VOID);
  }

  protected void fillWithGraphicsSize(
      int xf, int yf, int w, int h, int[] graphics, boolean bordered) {
  ...
  protected void fillWithGraphics(
      int xfrom,
      int xuntil,
      int yfrom,
      int yuntil,
    ...
    for (int i = yfrom; i <= yuntil; i++) {
      for (int j = xfrom; j <= xuntil; j++) {
        ...
        if (graphics[index] != Color.VOID.VALUE) {
          ...
          if (arrayindex < pixels.length && arrayindex >= 0) pixels[arrayindex] = color;
  ...
  protected int[] getBar(
      int width, int height, int startWidth, double percentage, Color color, Color color2) {
    if (percentage > 1) {
      percentage = 1;
    }
  ...
  protected void writeBar(
      int xFrom, int xUntil, int yFrom, int yUntil, double percentage, Color color, Color color2) {
