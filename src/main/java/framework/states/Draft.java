...
public class Draft extends State {
  ...
  public Draft(Memory memory) {
    super(memory);
    this.id = StateManager.DRAFT;
    this.hud = new HUD();
    this.hud.setDraft(this);
    if (this.memory.mode.equals(GameMode.PVP)) {
      this.memory.pvpRound++;
      this.initDraft();
    } else if (this.memory.mode.equals(GameMode.DUNGEON)) {
      this.memory.dungeonLevel++;
  ...
  private void finishDraft() {
    if (this.memory.mode.equals(GameMode.PVP)) {
      this.memory.teams[0].heroes = this.draftResult;
    } else if (this.memory.mode.equals(GameMode.DUNGEON)) {
      this.memory.dungeonTeam = new HeroTeam(1, this.draftResult, 1);
    }
    this.finished = true;
  }
  ...
  private void renderDraft() {
    ...
    //            fillWithGraphicsSize(x, y, hero.getWidth(), hero.getHeight(),
    // hero.render(Hero.DRAFT), this.draftPointer == i);
    //            x += hero.getWidth()+10;
    //        }
  }
