...
public class PvpDraft extends State {
    ...
    for (int i = 0; i < choices.length; i++) {
      ...
      if (hero != null) {
        ...
        fillWithGraphicsSize(
            x,
            y,
            Hero.draftDimensionX,
            Hero.draftDimensionY,
            hero.draftRender(),
            bordered || removed,
            bordered ? Color.WHITE : removed ? Color.RED : Color.VOID);
  ...
  private void renderTeams() {
    for (HeroTeam team : this.teams) {
      ...
      for (Hero hero : team.heroes) {
        fillWithGraphicsSize(
            x, y, hero.draftDimensionX, hero.draftDimensionY, hero.draftRender(), false);
